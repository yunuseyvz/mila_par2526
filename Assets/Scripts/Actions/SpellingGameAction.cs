...
namespace LanguageTutor.Actions
{
    public class SpellingGameAction : ILLMAction
    {
        ...
        /// <summary>
        /// Constructor: Initialize with optional word list from detected objects.
        /// If wordList is provided, picks random words from it.
        /// If empty, defaults to "CAT".
        /// </summary>
        public SpellingGameAction(List<string> detectedWords = null)
        {
            if (detectedWords != null && detectedWords.Count > 0)
            {
                wordList = new List<string>(detectedWords);
                // Pick first random word from the list
